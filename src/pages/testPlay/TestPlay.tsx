...
function MatchingQuestion({
  ...
  const rightColumn = questionsData["answer"].map(
            ...
            {leftColumn.map((item, index) => (
              <Paper
                ...
                sx={{
                  ...
                  marginBottom: 2,
                  cursor:
                    disabled || isLeftColumnDisabled(item)
                      ? "default"
                      : "pointer",
            ...
            {rightColumn.map((item, index) => (
              <Paper
                ...
                sx={{
                  ...
                  marginBottom: 2,
                  cursor:
                    disabled || isRightColumnDisabled(item)
                      ? "default"
                      : "pointer",
...
function TestPlay() {
  ...
  const handleNext = () => {
    if (currentQuestionIndex < questionsList.length - 1) {
      ...
      const correctAnswersCount = Object.values(submittedAnswers).filter(
  ...
  const handleSubmit = async () => {
    ...
    const convertAnswer = (answer) => {
      if (!isNaN(answer) && typeof answer !== "boolean")
    ...
    <Container sx={{ marginTop: 4 }}>
      ...
      >
        ...
        <Button
          ...
          onClick={handleNext}
          disabled={
            !isSubmitted || currentQuestionIndex >= questionsList.length
          }
        >
          {t("testPlay.next")}
        </Button>
